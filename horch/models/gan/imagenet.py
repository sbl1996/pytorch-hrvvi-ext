...
class ResNetGenerator(nn.Module):
    ...
    def __init__(self, in_channels, channels=64, out_channels=3, use_sn=True):
        ...
        self.dense = nn.Linear(in_channels, 4 * 4 * channels * 8)
        self.conv = nn.Sequential(
            ResBlock(channels * 8, channels * 8, 'up', use_sn=use_sn),
            ResBlock(channels * 8, channels * 4, 'up', use_sn=use_sn),
            ResBlock(channels * 4, channels * 4, 'up', use_sn=use_sn),
...
class ResNetDiscriminator(nn.Module):
    ...
    def __init__(self, in_channels=3, channels=64, out_channels=1, use_sn=True, use_bn=False):
        ...
        self.conv = nn.Sequential(
            ...
            ResBlock(channels * 4, channels * 4, 'down', use_sn=use_sn, use_bn=use_bn),
            ResBlock(channels * 4, channels * 8, 'down', use_sn=use_sn, use_bn=use_bn),
            ResBlock(channels * 8, channels * 8, 'down', use_sn=use_sn, use_bn=use_bn),
            nn.ReLU(True),
