...
void shape_check(at::Tensor input, at::Tensor offset, at::Tensor *gradOutput,
    ...
    AT_CHECK(dilationW > 0 && dilationH > 0,
             "dilation should be greater than 0, but got dilationH: %d "
             "dilationW: %d",
             dilationH, dilationW);
    ...
    if (outputWidth < 1 || outputHeight < 1)
        AT_ERROR("Given input size: (%ld x %ld x %ld). "
                 "Calculated output size: (%ld x %ld x %ld). Output size is "
                 "too small",
...
int deform_conv_forward_cuda(at::Tensor input, at::Tensor weight,
    ...
    for (int elt = 0; elt < batchSize / im2col_step; elt++) {
        ...
        columns =
            columns.view({group, columns.size(0) / group, columns.size(1)});
    ...
    output_buffer =
        output_buffer.view({batchSize / im2col_step, nOutputPlane, im2col_step,
                            outputHeight, outputWidth});
...
int deform_conv_backward_input_cuda(at::Tensor input, at::Tensor offset,
    ...
    if (input.ndimension() == 3) {
        ...
        offset =
            offset.view({1, offset.size(0), offset.size(1), offset.size(2)});
    ...
    gradInput =
        gradInput.view({batchSize, nInputPlane, inputHeight, inputWidth});
    columns = at::zeros(
        {nInputPlane * kW * kH, im2col_step * outputHeight * outputWidth},
        input.options());
...
int deform_conv_backward_parameters_cuda(
    ...
    for (int elt = 0; elt < batchSize / im2col_step; elt++) {
        ...
        columns =
            columns.view({group, columns.size(0) / group, columns.size(1)});
        gradWeight = gradWeight.view({group, gradWeight.size(0) / group,
...
void modulated_deform_conv_cuda_forward(
    ...
    for (int b = 0; b < batch; b++) {
        modulated_deformable_im2col_cuda(
            ...
            stride_w, dilation_h, dilation_w, deformable_group, columns);
...
void modulated_deform_conv_cuda_backward(
    ...
    columns =
        at::zeros({channels * kernel_h * kernel_w, height_out * width_out},
                  input.options());

    grad_output = grad_output.view({grad_output.size(0), group,
    ...
    for (int b = 0; b < batch; b++) {
        ...
        grad_weight = grad_weight.view(
            {group, grad_weight.size(0) / group, grad_weight.size(1),
        ...
        for (int g = 0; g < group; g++) {
            grad_weight[g] = grad_weight[g]
                                 .flatten(1)
                                 .addmm_(grad_output[b][g].flatten(1),
                                         columns[g].transpose(0, 1))
                                 .view_as(grad_weight[g]);
            if (with_bias) {
                grad_bias[g] = grad_bias[g]
                                   .view({-1, 1})
                                   .addmm_(grad_output[b][g].flatten(1),
                                           ones.view({-1, 1}))
                                   .view(-1);
