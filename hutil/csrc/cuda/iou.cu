...
iou_nm_forward_cuda(const at::Tensor &boxes1, const at::Tensor &boxes2) {
    AT_ASSERTM(input.device().is_cuda(), "boxes1 must be a CUDA tensor");
    AT_ASSERTM(rois.device().is_cuda(), "boxes2 must be a CUDA tensor");
...
__global__ void iou_nm_backward(T *dboxes1, T *dboxes2, const T *dout,
                                const T *boxes1, const T *boxes2, const int n,
                                const int m, const T *ious) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    int j = blockIdx.y * blockDim.y + threadIdx.y;
