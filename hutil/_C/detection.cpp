...
at::Tensor soft_nms_cpu_kernel(const at::Tensor &dets_t,
                          at::Tensor &scores_t,
                          const float iou_threshold, const int topk, const float score_threshold)
{
    AT_ASSERTM(!dets_t.type().is_cuda(), "dets_t must be a CPU tensor");
    AT_ASSERTM(!scores_t.type().is_cuda(), "scores must be a CPU tensor");
    AT_ASSERTM(dets_t.type() == scores_t.type(), "dets_t should have the same type as scores");
    ...
    at::Tensor indices_t = torch::empty({n}, at::device(at::kCPU).dtype(at::kLong));
                   ...
                   const float score_threshold)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(dets.type(), "softnms", [&] {
        result = soft_nms_cpu_kernel<scalar_t>(dets, scores, iou_threshold, topk, score_threshold);
    });
    return result;
}
